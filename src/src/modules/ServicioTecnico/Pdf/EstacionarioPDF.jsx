...
function EstacionarioPDF({ servicio }) {
    ...
    const generar = async () => {
        try {
            ...
            const fechaBackend = new Date(servicio.fechainicio);
            ...
            const bodyEquipo = [
                ["Marca", servicio.marca || "---", "Presión Descarga", lectura.presion_descarga ? `${lectura.presion_descarga} PSI` : "---"],
                ["Modelo", servicio.modelo || "---", "Temp. Descarga", lectura.temp_descarga ? `${lectura.temp_descarga} °C` : "---"],
                ["N° de Serie", servicio.serie || "---", "Amp. Motor Principal", lectura.amp_motor ? `${lectura.amp_motor} AMP` : "---"],
                ["Horómetro", lectura.horometro ? `${lectura.horometro} H` : "---", "Amp. Motor Ventilador", lectura.amp_motor_ventilador ? `${lectura.amp_motor_ventilador} AMP` : "---"],
                ...
                ["Presión Carga", lectura.presion_carga ? `${lectura.presion_carga} PSI` : "---", "Equipo en Operación", lectura.equipo_operacion || "---"]
            ...
            const actividadesConDato = todasLasActividades.filter(act => act[1] && act[1] !== "" && act[1] !== "-");
            ...
            const informeTecnico = (servicio.informe_tecnico && servicio.informe_tecnico.length > 0)
                ? servicio.informe_tecnico[0]
                : {};
            ...
            if (firmaValue) {
                ...
                if (typeof firmaSource === "string" && firmaSource.startsWith("/uploads/")) {
                    firmaSource = `${ApiWebURL.replace(/\/api\/?$/, '')}${firmaSource}`;
                } else if (typeof firmaSource === "string" && !firmaSource.startsWith("data:image") && !firmaSource.startsWith("http")) {
                    firmaSource = `${ApiWebURL.replace(/\/api\/?$/, '')}/uploads/${firmaSource.replace(/^\/?uploads\//, '')}`;
                }
