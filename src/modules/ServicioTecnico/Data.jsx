...
export const SERVICIOS_TECNICOS = [
  {
    ...
    subservicios: [
      { id: "TIPO_A", label: "TIPO A" },
      { id: "TIPO_B", label: "TIPO B" },
      { id: "TIPO_C", label: "TIPO C" },
      { id: "TIPO_D", label: "TIPO D" },
      { id: "TIPO_E", label: "TIPO E" },
      { id: "TIPO_F", label: "TIPO F" },
      { id: "TIPO_S", label: "TIPO S" }
    ]
  },
  {
    id: "MANTENIMIENTO_CORRECTIVO", // Corregido de CORRECTIVE para consistencia en español
    label: "Mantenimiento Correctivo",
    subservicios: [
      { id: "REPARACIONES", label: "Reparaciones" },
      { id: "CALIBRACIONES", label: "Calibraciones" }
    ]
  },
  {
    id: "PROTOCOLO_DE_PRUEBA",
    label: "Protocolo de Prueba",
    subservicios: [
      { id: "ARRANQUE_INICIAL", label: "Arranque Inicial" },
  ...
  },
  {
    ...
    subservicios: [
      ...
      { id: "EVALUACION_POR_FALLA", label: "Evaluación por Falla" },
  ...
  },
  {
    ...
    subservicios: [
      ...
      { id: "SISTEMA_DE_AIRE", label: "Sistema de Aire" },
      { id: "SISTEMA_DE_CONDENSADO", label: "Sistema de Condensado" },
      { id: "EQUIPOS", label: "Equipos" }
    ]
  },
  {
    id: "ACTIVIDAD_DE_APOYO",
    label: "Actividad de Apoyo",
    subservicios: [
      { id: "APOYO_TECNICO", label: "Apoyo Técnico" },
      { id: "APOYO_LOGISTICO", label: "Apoyo Logístico" }
    ]
  }
];
...
export const MAESTRO_COMPONENTES = filtros_y_componentes.reduce((acc, current) => {
  acc[current.id] = current.placeholder;
  return acc;
}, {});
// --- DEFINICIÓN EXPLICITA DE CHECKLISTS ESPECÍFICOS ---
const CHECKLIST_KAISHAN = [
  "filtroAirePrim", "kitPresMin", "kitValvTerm", "mangLub", "filtroAireSec",
  "filtroAceite", "kitRegAdm", "filtroSepPrim", "valvAlivio", "drenAutoSeca",
  "filtroSepSec", "kitValvAdm", "valvChkDesc", "anilloTanque", "lubricante",
  "valvChkCtrl", "filtLineCtrl", "orifRet", "kitSol2Vias", "trampAgua",
  "filtRet", "acopFlex", "carbonActAir", "enfrAceite", "preFiltCoal",
  "postFiltCoal", "tableroEquip", "conexMotor", "ventMotorPrin", "conexMotorSec",
  "ventMotorSec", "Condensador", "Evaporador", "fajaAccionamiento", "sistemaLubricacion"
];

const CHECKLIST_SULLAIR = [
  "filtroAirePrim", "kitPresMin", "kitValvTerm", "mangLub", "filtroAireSec",
  "filtroAceite", "kitRegAdm", "filtroSepPrim", "valvAlivio", "drenAutoSeca",
  "filtroSepSec", "kitValvAdm", "valvChkDesc", "anilloTanque", "lubricante",
  "valvChkCtrl", "filtLineCtrl", "kitSol2Vias", "trampAgua", "filtRet",
  "acopFlex", "carbonActAir", "enfrAceite", "preFiltCoal", "postFiltCoal",
  "tableroEquip", "conexMotor", "ventMotorPrin", "conexMotorSec", "ventMotorSec",
  "kitParAceite", "valvShut1", "kitRepEsp", "Condensador", "Evaporador",
  "kitRegEsp", "valvChk1", "kitSol3Vias", "fajaAccionamiento", "sistemaLubricacion"
];

const CHECKLIST_ATLAS_COPCO = [
  "filtroAirePrim", "kitPresMin", "kitValvTerm", "mangLub", "filtroAireSec",
  "filtroAceite", "filtroSepPrim", "valvAlivio", "drenAutoSeca", "filtroSepSec",
  "kitValvAdm", "valvChkDesc", "anilloTanque", "lubricante", "valvChkCtrl",
  "kitSol2Vias", "trampAgua", "filtRet", "acopFlex", "carbonActAir",
  "enfrAceite", "preFiltCoal", "postFiltCoal", "tableroEquip", "conexMotor",
  "ventMotorPrin", "conexMotorSec", "ventMotorSec", "Condensador", "Evaporador",
  "fajaAccionamiento", "sistemaLubricacion"
];

const CHECKLIST_KAESER = [
  "filtroAirePrim", "kitPresMin", "kitValvTerm", "mangLub", "filtroAireSec",
  "filtroAceite", "filtroSepPrim", "valvAlivio", "drenAutoSeca", "filtroSepSec",
  "kitValvAdm", "anilloTanque", "lubricante", "valvChkCtrl", "kitSol2Vias",
  "trampAgua", "acopFlex", "carbonActAir", "enfrAceite", "preFiltCoal",
  "postFiltCoal", "tableroEquip", "conexMotor", "ventMotorPrin", "conexMotorSec",
  "ventMotorSec", "Condensador", "Evaporador", "fajaAccionamiento", "sistemaLubricacion"
];


// --- CONFIGURACIÓN DE MAPEO REDUCIDA (Solo se configuran las excepciones) ---
export const CONFIGURACION_SERVICIOS = {
  "KAISHAN": CHECKLIST_KAISHAN,
  "SULLAIR": CHECKLIST_SULLAIR,
  "ATLAS COPCO": CHECKLIST_ATLAS_COPCO,
  "KAESER": CHECKLIST_KAESER
};

const MAPA_CHECKLISTS = {
  "KAISHAN": CHECKLIST_KAISHAN,
  "SULLAIR": CHECKLIST_SULLAIR,
  "ATLAS COPCO": CHECKLIST_ATLAS_COPCO,
  "KAESER": CHECKLIST_KAESER
};
// --- Asegúrate de importar estas constantes si están en otro archivo ---
// Asegúrate de tener los espacios reales aquí
const TIPOS_MANTENIMIENTO_CON_CHECKLIST = ["TIPO A", "TIPO B", "TIPO C", "TIPO D", "TIPO F"];

/**
 * Obtiene los componentes/filtros de manera universal para CUALQUIER servicio o subservicio.
 * @param {string} marca - Nombre de la marca (ej. "Sullair", "Kaeser")
 * @returns {Array} Bloque de componentes filtrados que corresponden a la marca
 */
export const obtenerComponentesPorServicio = (marca) => {
  // 1. Validar que la marca no venga vacía o nula
  if (!marca) return [];

  // 2. Normalizar la marca a mayúsculas y quitar espacios
  const marcaKey = marca.toUpperCase().trim();

  // 3. Obtener la lista de IDs asignados a la marca (Si no existe, usa KAISHAN por defecto)
  const listaIds = CONFIGURACION_SERVICIOS[marcaKey] || CONFIGURACION_SERVICIOS["KAISHAN"];

  // 4. Retornar los objetos completos mapeados desde 'filtros_y_componentes'
  return filtros_y_componentes.filter(item => listaIds.includes(item.id));
};